...
function enviarDados() {
    ...
    // Dados adicionais da outra página
    const itens = JSON.parse(obterParametroDaURL('itens'));
    ...
    // Enviar os dados para o e-mail usando a API já presente no código
    fetch('https://formsubmit.co/jumbex.jumboexpress@gmail.com', {
        ...
        body: JSON.stringify({
            ...
            sobrenome: sobrenome,
            whatsapp: whatsapp,
            // Adicionar outras informações obtidas da página anterior, se necessário
            itens: itens,
    ...
    .then(response => {
        if (response.ok) {
            ...
            // Aqui você pode adicionar qualquer ação após o envio bem-sucedido, se necessário
        } else {
            console.error('Erro ao enviar os dados para o e-mail!');
            // Lidar com o erro, exibir mensagem ou tomar outra ação, se necessário
        }
    })
    .catch(error => {
        console.error('Erro:', error);
