...
function validarCampos() {
    ...
    const nomeFamiliar = document.getElementById('Nome do Seu familiar').value; // Obtendo o valor do campo "Nome do Seu familiar"
    const unidadePrisional = document.getElementById('opcoes').value; // Obtendo o valor selecionado na lista da unidade prisional

    const regexLetras = /^[a-zA-ZÀ-ú\s]+$/;

    if (nome.trim() === '' || sobrenome.trim() === '' || whatsapp.trim() === '' || nomeFamiliar.trim() === '' || unidadePrisional === 'opcao0') {
        alert('Por favor, preencha todos os campos.');
    } else if (!regexLetras.test(nome) || !regexLetras.test(sobrenome) || !regexLetras.test(nomeFamiliar)) {
        alert('Por favor, insira apenas letras nos campos de nome, sobrenome e nome do familiar.');
    } else {
        const telefoneNumerico = whatsapp.replace(/\D/g, '');
        if (telefoneNumerico.length !== 11) {
            alert('O número de telefone (WhatsApp) deve conter 11 dígitos.');
        } else {
            const confirmacao = confirm(`Por favor, confirme suas informações:\n\nNome: ${nome}\nSobrenome: ${sobrenome}\nWhatsApp: ${whatsapp}\nNome do Familiar: ${nomeFamiliar}\nUnidade Prisional: ${unidadePrisional}\n\nAs informações estão corretas?`);
  
            if (confirmacao) {
                const detalhesCompra = encodeURIComponent(document.getElementById('detalhesCompra').innerHTML);
                const urlEnvio = `envio.html?nome=${encodeURIComponent(nome)}&sobrenome=${encodeURIComponent(sobrenome)}&whatsapp=${encodeURIComponent(whatsapp)}&nomeFamiliar=${encodeURIComponent(nomeFamiliar)}&unidadePrisional=${encodeURIComponent(unidadePrisional)}&detalhesCompra=${detalhesCompra}`;
